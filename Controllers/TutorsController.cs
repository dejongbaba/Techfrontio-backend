...
namespace Course_management.Controllers
{
    ...
    public class TutorsController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetDashboard()
        {
            ...
            // Calculate total amount made from completed payments for tutor's courses
            var courseIds = courses.Select(c => c.Id).ToList();
            decimal totalAmountMade = 0m;
            if (courseIds.Count > 0)
            {
                totalAmountMade = await _context.Payments
...
// GET api/tutors/{id} - Get tutor by ID (public)
        ...
        // GET api/tutors/students - Get all students enrolled across tutor's courses
        [HttpGet("students")]
        [Authorize(Roles = "Tutor")]
        public async Task<IActionResult> GetAllStudents()
        {
            ...
            if (!courseIds.Any())
            {
                return Ok(ApiResponse<List<EnrollmentDto>>.Success(new List<EnrollmentDto>(), "No courses found for tutor", 200));
            }
            ...
            return Ok(ApiResponse<List<EnrollmentDto>>.Success(enrollmentDtos, "Students retrieved successfully", 200));
        }
    }
}
